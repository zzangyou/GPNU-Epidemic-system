module.exports = class student_mod extends require('./model') {
     ...
     * @param kill
     */
    static sethealthMod(u_id, temperature, hot, goother, otheradd, fever, leave, hesuan, mask, masknum, kill) {
        return new Promise((resolve, reject) => {
            let sql = "insert into health(u_id, temperature, hot, goother, otheradd, fever, leaveout, hesuan, mask, masknum, kills) values (?,?,?,?,?,?,?,?,?,?,?)"
            this.query(sql, this.formatParams(u_id, temperature, hot, goother, otheradd, fever, leave, hesuan, mask, masknum, kill))
                .then(result => {
                    resolve("健康填报表提交成功")
                }).catch(err => {
                    reject("健康填报表提交失败")
